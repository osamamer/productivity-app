...
export function StatComposerSheet({ visible, onClose, onCreated }: {
  ...
  return (
    <ModalSheet visible={visible} onClose={close} title="New stat" footer={<AppButton label="Create stat" loading={saving} onPress={() => void submit()} />}>
      ...
      <ChoiceChips value={type} onChange={setType} options={[{ value: 'BOOLEAN', label: 'Yes / No' }, { value: 'NUMBER', label: 'Number' }, { value: 'RANGE', label: 'Range' }, { value: 'TIME', label: 'Time of day' }, { value: 'DURATION', label: 'Duration' }]} />
